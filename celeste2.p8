...
function player.update(self)
  ...
  local against_wall = (self:check_solid(-2) and input_x == -1)  or (self:check_solid(2) and input_x == 1)
	if on_ground then
		self.t_jump_grace = 4
	...
	if self.state == 0 then
		...
		-- gravity
		if not on_ground and not against_wall then
	...
	for o in all(objects) do
		...
		elseif o.base == crumble and not o.breaking then
			...
			if self.state == 0 and self:overlaps(o, 0, 1) then
				o.breaking = true
				psfx(8, 20, 4)
      elseif self.state ==3 and self:overlaps(o, 1, 1) then
				o.breaking = true
				psfx(8, 20, 4)
      end
